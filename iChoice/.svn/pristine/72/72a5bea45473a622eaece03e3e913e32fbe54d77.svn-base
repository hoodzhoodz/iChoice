...
public class C218RGattCallback extends BaseGattCallback {
    ...
    //Feature
    private static final UUID CDF2 = UUID
        ...
        public void handleMessage(Message msg) {
            ...
            switch (msg.what) {
                ...
                case PULSE_OX_FEATURE_SERVICE://血氧脉率服务
                    readCharacteristic(gatt, service, CDF2);
                    break;
                default:
                    Log.d(TAG, "default:");
                    break;
    ...
    private boolean supportedRR = true;//呼吸率
...
//            onDataReceivedSN(characteristic.getValue());
        ...
        if (characteristic.getUuid().equals(CDF2)) {//读血氧特征值成功
    ...
    private void sendFeature() {
        ...
        map.put("supportedRR", supportedRR);
        map.put("supportedRRInterval", supportedRRInterval);
        map.put("supportedRRArterialState", supportedRRArterialState);
...
//        43002000240800
        ...
        String featureLow = ByteUtils.hexStringReverse(ByteUtils.bytes2HexString(data).substring(0, 2));
        String featureHigh = ByteUtils.hexStringReverse(ByteUtils.bytes2HexString(data).substring(2, 4));
        String binary = ByteUtils.hexString2binaryString(featureHigh + featureLow);
        Log.d(TAG, "binary:" + binary);
        StringBuilder builder = new StringBuilder(binary);
        builder.reverse();
        Log.d(TAG, "builder:" + builder);
