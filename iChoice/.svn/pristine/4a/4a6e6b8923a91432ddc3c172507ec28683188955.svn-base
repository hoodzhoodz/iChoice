...
public class CalenderSelectActivity extends AppCompatActivity implements
    ...
    private void initDate() {
        ...
        switch (bundle.getInt("type")) {
            ...
            case DevicesType.Ecg:
                initEcg();
                widget.addDecorator(new EventDecorator(getResources().getColor(R.color.blue_098bfa), calendarDays));
                break;
            default:
        }
        initBeginTime();
    ...
    private void initEcg() {
        ...
        if (deviceDisplay != null && deviceDisplay.getEcg() != 0) {
            List<EcgData> ecgDataList = getDaoSession(this).getEcgDataDao().queryBuilder().where(EcgDataDao.Properties.UserId.eq(IchoiceApplication.getAppData().userProfileInfo.getUserId())).list();
            if (!ecgDataList.isEmpty()) {
                for (EcgData ecgData : ecgDataList) {
                    String DataDate = ecgData.getMeasureTime();
