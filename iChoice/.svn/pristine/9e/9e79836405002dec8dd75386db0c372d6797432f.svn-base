...
public class C218RBleConService extends Service {
    public static final String TAG = "C218RBleConService";
    private List<DeviceInfo> list;
    Subscription c208Subscription;
    C208 c208s;
    C218RInvoker c218RInvoker;
    C218RBleConService.ScanBleBinder binder = new ScanBleBinder();
    ...
    private boolean supportedRRInterval = true;//呼吸率间期
    private boolean supportedRRArterialState = true;//动脉状态
    ...
    private boolean continuousRRInterval = true;
    private boolean continuousRRArterialState = true;
    ...
    private void initSpotCheckState() {
        ...
        deviceClockSet = true;
        spotCheckRR = true;
        supportedRRInterval = true;
        supportedRRArterialState = true;
    }

    public C218RBleConService() {
    }
    ...
    public class ScanBleBinder extends Binder {
        ...
        public void cancelConnect() {
            ...
            c218RInvoker.cancelConnectDevice(new C208CancelConnectCallback() {
                @Override
        ...
        public void disconnect() {
            ...
            c218RInvoker.disConnectDevice(new C208DisconnectCallBack() {
                @Override
                public void onSuccess() {
    ...
    private void openBluetoothBle() {
        BluetoothManager bluetoothManager = (BluetoothManager) C218RBleConService.this.getSystemService(Context.BLUETOOTH_SERVICE);
    ...
    private void connectC208(final C208 c208s) {
        c218RInvoker.connectDevice(new C218RConnectAandDataResponseCallback() {
            @Override
...
//                2c 6200 3f00 2000 000035
                    ...
                    if (continuousFlag[6] != '1') {
                        continuousRRInterval = false;
                        Log.d(TAG, "实时数据不支持呼吸率间期");
                    }
                    if (continuousFlag[7] != '1') {
                        continuousRRArterialState = false;
                        Log.d(TAG, "实时数据不支持呼吸率动脉状态");
                    }
                    ...
                    if (continuousPI) {
                        dataSource = ByteUtils.hexStringReverse(continuousData.substring(continuousData.length() - 14, continuousData.length() - 10));
                        DSS = ByteUtils.hexStringReverse(continuousData.substring(continuousData.length() - 10, continuousData.length() - 4));
                        byte high = data[data.length - 1];
                        ...
                        dataSource = ByteUtils.hexStringReverse(continuousData.substring(continuousData.length() - 10, continuousData.length() - 6));
                        DSS = ByteUtils.hexStringReverse(continuousData.substring(continuousData.length() - 6));
                    }
            ...
            public void onRACPResponse(DeviceType deviceType, byte[] result) {
                ...
                if (lastCMD.equals(reportNumberOfStoredRecordsCmd)) {
                    ...
                    c218RInvoker.sendCmd(reportStoredRecordsCmd);
                    return;
                }
                ...
                if (lastCMD.equals(reportStoredRecordsCmd)) {
                    ...
                    c218RInvoker.sendCmd(deleteStoredRecordsCmd);
                    Log.d(TAG, "获取历史记录成功！");
                    return;
            ...
            public void onSuccess() {
                ...
                if (feature.get("RACP")) {
                    ...
                    c218RInvoker.sendCmd(reportNumberOfStoredRecordsCmd);
                }
    ...
    private void analysisPointData(byte[] data) {
        ...
        if (spotCheckFlag[6] != '1') {
            supportedRRInterval = false;
            Log.d(TAG, "点测数据不支持呼吸率间期");
        }
        if (spotCheckFlag[7] != '1') {
            supportedRRArterialState = false;
            Log.d(TAG, "点测数据不支持呼吸率动脉状态");
        }
...
//        }
        ...
        String DSS;
        if (spotCheckPI)
            DSS = ByteUtils.hexStringReverse(spotCheckData.substring(spotCheckData.length() - 10, spotCheckData.length() - 4));
        else
            DSS = ByteUtils.hexStringReverse(spotCheckData.substring(spotCheckData.length() - 6));
        StringBuilder DSSBuilder = new StringBuilder(ByteUtils.hexString2binaryString(DSS));
        DSSBuilder.reverse();
        char[] deviceSensorStatus = DSSBuilder.toString().toCharArray();
        if (deviceSensorStatus[2] == '1')
        ...
        String dataSource;
        if (spotCheckPI)
            dataSource = ByteUtils.hexStringReverse(spotCheckData.substring(spotCheckData.length() - 14, spotCheckData.length() - 10));
        else
            dataSource = ByteUtils.hexStringReverse(spotCheckData.substring(spotCheckData.length() - 10, spotCheckData.length() - 6));
        StringBuilder dataSourceBuilder = new StringBuilder(ByteUtils.hexString2binaryString(dataSource));
        dataSourceBuilder.reverse();
        char[] measurementStatus = dataSourceBuilder.toString().toCharArray();
