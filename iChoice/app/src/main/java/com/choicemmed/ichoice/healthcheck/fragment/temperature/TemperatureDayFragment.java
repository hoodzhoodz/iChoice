...
public class TemperatureDayFragment extends BaseFragment {
    ...
    public void onClick(View view) {
        ...
        switch (view.getId()) {
            ...
            case R.id.calender_select:
                ...
                String current_date = DateUtils.getDateToString(calendar.getTimeInMillis(), FormatUtils.template_DbDateTime);
    ...
    private void CalendarMoveChart(Calendar endCalendar) {
        ...
        LogUtils.d(TAG, diff + "");
        currentStartXvalue = diff * 24;
        temperature_chart.moveViewToX(diff * 24f);
    }
    ...
    public void initData() {
        ...
        xValueMax = DateUtils.differentDayCalendar(beginCalendar, endCalendar) * 24;
    }
...
//                MPPointD topLeft = temperature_chart.getValuesByTouchPoint(handler.contentLeft(), handler.contentTop(), YAxis.AxisDependency.LEFT);
                ...
                LogUtils.d(TAG, " (bottomRight)  " + (bottomRight.x));
//                去除误差-0.01 不计算明天
                calendar.setTimeInMillis((((long) (bottomRight.x - 0.01) * 1000 * 60 * 60) + beginCalendar.getTimeInMillis()));
                setTextDate(calendar);
                calculationCurrentDayLastValue();
                return false;
            }
        ...
        temperature_chart.setVisibleXRange(0, 24);
    ...
    private void setChartData() {
        List<CFT308Data> cft308DataList = temperatureOperation.queryAVGData(IchoiceApplication.getAppData().userProfileInfo.getUserId());
        if (!cft308DataList.isEmpty()) {
            for (int i = 0; i < cft308DataList.size(); i++) {
                ...
                int startX = (int) ((calendar1.getTimeInMillis() - beginCalendar.getTimeInMillis()) / (nh));//计算差多少小时
    ...
    private void calculationCurrentDayLastValue() {
        if (cft308DataHashMapC.size() > 0) {
            String everyDayLastMeasureTime = FormatUtils.getDateTimeString1(calendar.getTimeInMillis(), template_Date);
            ...
            if (cft308Data != null) {
                ...
                times.setText(calendar1.get(Calendar.HOUR_OF_DAY) + "h");
